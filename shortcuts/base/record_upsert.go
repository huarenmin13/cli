...
var BaseRecordUpsert = common.Shortcut{
	...
	Tips: append([]string{
		...
		"Sub-record/child-record path: when a one-way/two-way link field represents hierarchy, create a normal record and set that link field to the parent record ID array, e.g. {\"Parent Link\":[\"rec_xxx\"]}; do not look for parent_record_id or a separate child-record API.",
