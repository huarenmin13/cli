...
5. 主计算、校验和最终答案复用同一口径；口径变更后重新计算受影响结果。替代口径只能作为明确标注的补充或诊断，不得静默替换数据源、字段、过滤条件、日期或公式，也不能混入主趋势和主结论。
...
时间序列从用户请求的范围和粒度生成相邻、不重叠的周期，并保留该范围内的空桶。使用 `+data-query` 时，日期 operator、`ExactDate` 和边界能力以 [lark-base-data-query.md](lark-base-data-query.md) 为准；能力不足时标记证据缺口，不得通过猜测时区或边界补出结果。
...
- 比率或比值分别定义分子和分母的数据源、统计总体、过滤条件及对齐维度；若指标定义为某总体中的命中率，分子总体应是分母总体的子集。分母为 0 时输出“无数据/不适用”，不能写成 0%；存在多个合理分母时先消歧，不得自行补全业务定义。
- 校验方式匹配聚合函数：互斥且覆盖完整范围的 `sum` / `count` / `count_all` 可将分桶值求和；`avg` 使用 `sum + count` 加权或独立做全区间聚合；`min` / `max` 比较各桶极值；`distinct_count` 独立计算全区间，不能相加；比率用汇总后的分子与分母重算。所有比较使用未舍入值。
