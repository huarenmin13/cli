...
连续日历区间分桶时，从用户请求的范围和粒度生成相邻、不重叠的周期并保留空桶；空桶输出 0、无数据或约定填充值，由已锁定的度量和公式决定。使用 `+data-query` 时，日期 operator、`ExactDate` 和边界能力以 [lark-base-data-query.md](lark-base-data-query.md) 为准；能力不足时标记证据缺口，不得通过猜测时区或边界补出结果。
