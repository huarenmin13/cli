...
- 当问题涉及日期筛选、时间维度或自然时间范围，且用户未指定日期字段时，按问题中的业务事件选择候选日期字段并验证非空覆盖；不得仅因一个候选字段为空就判定无记录。多个候选会改变结论时，说明所用字段或请求澄清。非时间分析不得为满足本规则引入日期字段或时间过滤。
- 同一结果的分期数量、比率和趋势必须复用同一时间字段、统计总体和分母；用户未明确要求时不得增加第二套比率口径。
- 需要多次查询或关联后汇总时，各查询、关联、派生计算和最终结论必须复用同一统计总体、筛选和排除口径。
...
涉及分期、比率或趋势时，执行查询前先明确统计总体、时间轴、分子、分母和排除条件：

- 用户指定时间字段或计算公式时严格使用。未指定时，记录规模、转化率及其分子数量按记录进入统计总体的创建／录入时间分桶；独立的事件发生量按实际事件时间分桶，不得用计划、预计或截止日期代替实际事件时间。
- 用户未定义比率分母时，以同一时间桶、同一统计总体中的全部记录为分母；仅用于判断分子是否命中的状态或结果条件不得同时过滤分母。分母为 0 时输出“无数据”，不要计算成有效百分比。
- 用户指定范围时严格覆盖。未指定范围时，先查询数据覆盖范围并声明采用的完整范围；若数据只落在一个自然年，月／季度分组覆盖该自然年。查询后补齐已声明范围内所有空桶，分别输出 0、0 和“无数据”。
...
- 如果涉及时间序列，是否逐桶输出总数、命中数和比率，补齐已声明范围内的空桶，并在表后给出同口径的总体分子、总体分母、总体比率和只依据该主序列得出的趋势结论。
