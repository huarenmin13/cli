...
涉及分期、比率、趋势或多次查询时，在首个业务数据查询前固定统计总体、时间轴、分子、分母、排除条件和完整时间范围；同一回答中的查询、关联、派生计算和最终结论必须复用同一口径。
...
- 最终表逐桶输出总数、命中数和比率。用户未指定范围且数据只落在一个自然年时，月／季度分组覆盖该自然年；其他情况先声明范围。补齐范围内所有空桶（0、0、“无数据”），表后给出同口径的总体分子、总体分母、总体比率，并只依据这套主序列下趋势结论。除非用户明确要求，不要主动增加第二套比率口径。
