...
写 jq、Python 或 DSL 前，先用一小段注释固定本次计算合同；最终结论也必须沿用同一合同：

1. **范围与粒度：** 写明分析对象、筛选范围、目标粒度和分组维度。用户指定的字段或公式原样优先；存在多个候选日期或分类字段时，先确定主分组维度组合并说明选择依据，其他候选只能作为清楚标注的补充。若不同选择会实质改变结论且无法从用户要求或 schema 消歧，先询问用户。
2. **度量与比率：** 写明直接度量字段、聚合方式和单位；比率必须写成 `分子 / 分母`，不得静默从分母排除仍属于用户所述分析对象的记录。更窄的适用集合必须有用户要求、字段/schema、可信 View 或明确业务契约支持，否则只可作为替代口径单列，不能覆盖主口径。
3. **忠实计算：** 直接度量按目标粒度聚合，不得为了让它与另一字段或另一张表的总额一致而自行缩放、换算或分摊。只有用户给出的公式、字段定义、关系 schema 或已确认业务契约才能授权这类转换；总量守恒是发现冲突的校验，不是发明转换的依据。
4. **完整输出轴：** 用户给出有限日期范围及日、周、月、季度或年度粒度时，先生成范围内的完整时间轴再左连接聚合结果。按月或季度比较只涉及一个已结束自然年且用户未限定更窄区间时，完整轴覆盖该年 1–12 月或 Q1–Q4。无记录周期的计数为 `0`；比率为 `0 / 0` 时标记 `N/A`（无数据），不得省略周期或伪造有效百分比。
5. **冲突裁决：** 主合同先算、先验收、先交付。补充维度、替代分母或第二种分析引擎产生不同结果时，回到完整导出、边界和合同逐项解释差异；不能以结果更新、数值更大或更符合直觉为由让后算出的结果推翻已验收的主结果。

最终输出至少标明主维度、度量或分子/分母以及查询范围。替代口径可以帮助解释敏感性，但必须与主结果分区展示，且不得用它给出相反的主趋势判断。

### 主结果隔离（存在竞争口径时）

当确定性分析存在多个候选分组、分母、公式或分析引擎，且不同选择会实质改变结论时，可以使用一个 `primary_result` 作为内部隔离结构；简单、单一口径统计直接按上方计算合同执行，不要求额外包装。使用该结构时：

1. **`primary_result.contract`：** 保存范围、主分组字段、用户请求的度量，以及适用的公式、单位和有限完整轴，并在生成主结果行前冻结。新证据确需改变任一项时，废弃旧对象并重新计算；不得在现有对象上换名、换字段或交换主/替代口径。
2. **`primary_result.rows`：** 每个结果行保存分组键和可复算的请求度量；比率任务才保存分子、分母与比率。用户明确要求或合同定义有限完整轴时，零记录与 `0 / 0` 仍保留，不能只在推理中保留。
3. **`primary_result.total`：** 只在度量互斥且可加时交叉复算；比率还要断言 `total.numerator == sum(rows[].numerator)`、`total.denominator == sum(rows[].denominator)` 并重算总比率。分组重叠、度量不可加或任务不需要合计时省略 `total`；若为审计保留该字段，则写 `total.status=not_applicable` 及原因，不伪造合计。
4. **`primary_result.trend`：** 只在任务要求趋势且轴有序时从主结果行派生；比率趋势断言 `trend == trend(rows[].rate)`，`N/A` 比较时排除，不能改成有效的零值。没有趋势请求或轴无序时省略 `trend`；若为审计保留该字段，则写 `trend.status=not_applicable` 及原因。

`primary_result` 在内部完整保留以供验收；最终回答只按用户请求投影必要行、合计与趋势，不默认展开高基数明细，正文数字不得另行心算。

候选维度、更窄适用集合或第二种公式分别写入 `alternatives`，不能回写或重命名 `primary_result`。若候选趋势冲突且用户要求或可信证据无法消歧，逐项输出 `contract → result` 并明确未消歧，不得输出一个由任一替代结果驱动的无条件单一趋势结论。

### 两次验收门禁

1. **计算前——输入证明：** 对以 `+record-list` / `+record-search` NDJSON 作为本地计算输入、且需要覆盖完整查询范围的任务，逐表或逐查询列出输入清单；只有该输入的全部页都已纳入，且最后一页 Record manifest 明示 `has_more=false`，才可在本地聚合或 JOIN。一项仍为 `has_more=true` 或状态未知就停止，继续收敛谓词或转 Cloud；不能用另一项的完成状态代替。用户明确将范围限定为前 N 条或预览样本时，把该有限范围写入计算合同并明确结果不得外推，不要求表尾 `has_more=false`。`+data-query` 已在云端完成聚合、排序或限制时，不要求 Record manifest；按 [Cloud SOP](lark-base-record-query-and-analysis-cloud-sop.md) 验收 DSL、筛选、维度、度量与结果范围。若随后回查逐条原始记录，回查产生的 NDJSON 仍按前述完整性门禁验收。
2. **交付前——合同对账：** 对照用户任务、计算合同、计算结果和候选回答，逐项核验真实 schema 字段或公式、范围与完整时间轴、主分组与补充维度、聚合或分子/分母、单位。缺项、改名、换口径或数字与趋势不一致时，先重算或改答。
3. 用户概念词同时匹配多个 schema 字段且没有证据消歧时，不按“惯例”指定权威字段；能询问就先询问，只读且无法交互时并列展示候选结果并标明未消歧。
4. 使用补充维度解释结论时，输出真实字段名及各值计数；不把多个真实值改写为推断的总括标签。
5. 使用 `primary_result` 时，交付从该对象投影，并复核 contract、rows 及仅在适用时存在的 total、trend；简单、单一口径统计则从已验收的计算结果按合同投影。候选回答若改换主标签、遗漏合同要求的轴或让合计、趋势脱离主 rows，停止交付并修正。
