...
4. 聚合或派生分析允许先读取 schema 和有限样本探索；在第一次用于支撑结论的计算前，按每个输出序列锁定数据源、统计总体、度量与聚合、分组，以及仅在适用时锁定时间口径和公式。用户明确指定的口径优先；多个合理候选会实质影响结果时，先说明差异并向用户消歧，不得仅凭字段名称、样本非空情况或“常见业务口径”建立通用优先级。当前交互无法完成消歧时，仅并列呈现会实质改变结论的最小候选集，不得自行选定唯一主口径。
...
- 当用户要求覆盖完整周期的连续分期结果时，最终交付同时给出同口径的全区间汇总；加性度量汇总分桶值，比率以全区间分子和分母按已锁定公式重算，其他不可由分桶结果安全合成的聚合按前述规则独立计算。
