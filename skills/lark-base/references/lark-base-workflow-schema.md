...
> **StepType 边界。** 新增节点的 `type` 只能使用本 Schema 或官方服务文档明确记录的值，不得根据自然语言发明或试探。`+workflow-get` 返回的未知 `type` 在 read-modify-write 中必须原样保留；无法确认能力时，说明边界并停止写入。`--dry-run` 只预览请求，不证明服务端支持。
>
> **已知不支持。** `DeleteRecordTrigger` 当前不可用，不要构造或提交。
>
> **替代方案需明确选择。** 能力不可用时，保持用户原始意图并停止写入。改变触发条件、数据结构或业务结果的方案只能先建议；用户明确选择后才能执行。请求失败、沉默或要求减少追问均不构成授权。
