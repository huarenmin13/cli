...
1. 分析用户需求：受众、目的、范围、**写作类型**。若命中某垂类写作类型（影评/种草/新闻评论/短视频脚本等），先查 [`../../writing-types/README.md`](../../writing-types/README.md) 索引（中文类型名→英文文件），再读对应 `writing-types/<文件>` 取写作知识与飞书格式差量。（公文等 Word 类型已由顶层路由到 office-word，不在本模块。）
2. 设计大纲：结合写作类型 guide（若有）与任务自然选择结构。可以是短文、纪要、FAQ、方案、报告、清单或其他形式；不要默认套固定章节、固定开头或固定富 block 配比。**写作类型只定"写什么"；飞书的 callout/编号/克制红线仍按 [`lark-doc-style.md`](lark-doc-style.md)**
3. `docs +create` 创建并撰写：
