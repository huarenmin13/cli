...
- `href` points to the chart file. Standard local SVG decks use `.svg` rendered from `assets/charts/specs/*.vl.json` by the local Node renderer (`vega-lite` + `vega`). Legacy `.chart` references are allowed only for imported PPTX preservation; preserve them as-is unless the user asks to change.
