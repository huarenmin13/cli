...
- TestBaseRecordHistoryListDryRunUsesExplicitRecordID / TestBaseRecordHistoryListHelpShowsConfirmedRowGuard: prove the history request keeps the explicit record ID and that command help requires a user-confirmed row, provides candidate recovery guidance, and exposes opt-in `--format pretty` output.
...
| ✓ | base +record-history-list | shortcut | base_record_history_dryrun_test.go::TestBaseRecordHistoryListDryRunUsesExplicitRecordID; TestBaseRecordHistoryListHelpShowsConfirmedRowGuard | `--base-token`; `--table-id`; `--record-id`; `--page-size`; `--format pretty`; dry-run + help | request shape, confirmed-row recovery guidance, and pretty-output discoverability |
