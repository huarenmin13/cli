...
- TestBaseRecordHistoryListDryRunUsesExplicitRecordID / TestBaseRecordHistoryListHelpShowsSelectionGuidance: prove the history request keeps the explicit record ID and that command help points to the record-selection workflow without claiming runtime confirmation, while exposing opt-in `--format pretty` output.
...
| ✓ | base +record-history-list | shortcut | base_record_history_dryrun_test.go::TestBaseRecordHistoryListDryRunUsesExplicitRecordID; TestBaseRecordHistoryListHelpShowsSelectionGuidance | `--base-token`; `--table-id`; `--record-id`; `--page-size`; `--format pretty`; dry-run + help | request shape, selection-workflow guidance, and pretty-output discoverability |
